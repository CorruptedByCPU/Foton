...
struct STD_STRUCTURE_FILE lib_vfs_file( struct LIB_VFS_STRUCTURE *vfs, uint8_t *path, uint64_t length ) {
	...
	while( TRUE ) {
		...
		// file found?
		if( ! vfs -> length ) return (struct STD_STRUCTURE_FILE) { EMPTY };	// file not found

		// last file from path is requested one?
		if( length == filename_length && lib_string_compare( path, (uint8_t *) vfs -> name, filename_length ) ) return (struct STD_STRUCTURE_FILE) { (uint64_t) vfs, vfs -> size, vfs -> type, vfs -> mode, vfs -> uid, vfs -> guid };
