...
uint8_t kernel_network_udp( struct KERNEL_STRUCTURE_NETWORK_HEADER_ETHERNET *ethernet, uint16_t length ) {
	...
	struct KERNEL_STRUCTURE_NETWORK_HEADER_UDP *udp = (struct KERNEL_STRUCTURE_NETWORK_HEADER_UDP *) ((uintptr_t) ipv4 + ipv4_header_length);
	...
	for( uint64_t i = 0; i < KERNEL_NETWORK_SOCKET_limit; i++ ) {
		...
		// amount of data
		uint64_t bytes = length - ((uintptr_t) udp - (uintptr_t) ethernet) - sizeof( struct KERNEL_STRUCTURE_NETWORK_HEADER_UDP );

		// move data content at beginning of area
		uint8_t *data = (uint8_t *) udp + sizeof( struct KERNEL_STRUCTURE_NETWORK_HEADER_UDP );
...
void kernel_network_udp_exit( struct KERNEL_STRUCTURE_NETWORK_SOCKET *socket, uint8_t *data, uint64_t length ) {
	...
	// copy data
	uint8_t *udp_data = (uint8_t *) ((uintptr_t) udp + sizeof( struct KERNEL_STRUCTURE_NETWORK_HEADER_UDP ));
	for( uint16_t i = 0; i < length; i++ ) udp_data[ i ] = data[ i ];

	// encapsulate UDP frame and send
	kernel_network_udp_encapsulate( socket, ethernet, sizeof( struct KERNEL_STRUCTURE_NETWORK_HEADER_UDP ) + length );
}
...
void kernel_network_udp_encapsulate( struct KERNEL_STRUCTURE_NETWORK_SOCKET *socket, struct KERNEL_STRUCTURE_NETWORK_HEADER_ETHERNET *ethernet, uint16_t length ) {
	...
	struct KERNEL_STRUCTURE_NETWORK_HEADER_UDP *udp = (struct KERNEL_STRUCTURE_NETWORK_HEADER_UDP *) ((uintptr_t) ethernet + sizeof( struct KERNEL_STRUCTURE_NETWORK_HEADER_ETHERNET ) + sizeof( struct KERNEL_STRUCTURE_NETWORK_HEADER_IPV4 ));
	udp -> local = MACRO_ENDIANNESS_WORD( socket -> port_local );
	udp -> target = MACRO_ENDIANNESS_WORD( socket -> port_target );
	udp -> length = MACRO_ENDIANNESS_WORD( (length + sizeof( struct KERNEL_STRUCTURE_NETWORK_HEADER_UDP)) );

	// properties of UDP Pseudo header
	struct KERNEL_STRUCTURE_NETWORK_HEADER_PSEUDO *pseudo = (struct KERNEL_STRUCTURE_NETWORK_HEADER_PSEUDO *) ((uintptr_t) udp - sizeof( struct KERNEL_STRUCTURE_NETWORK_HEADER_PSEUDO ));
	...
	udp -> checksum = kernel_network_checksum( (uint16_t *) pseudo, sizeof( struct KERNEL_STRUCTURE_NETWORK_HEADER_PSEUDO ) + MACRO_ENDIANNESS_WORD( udp -> length ) );

	// wrap data into a IPv4 frame and send
	kernel_network_ipv4_encapsulate( socket, ethernet, length + sizeof( struct KERNEL_STRUCTURE_NETWORK_HEADER_UDP ) );
}