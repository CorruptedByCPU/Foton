...
struct	DRIVER_PCI_STRUCTURE driver_pci_find_class_and_subclass( uint16_t class_and_subclass ) {
	struct DRIVER_PCI_STRUCTURE pci = { EMPTY, EMPTY, EMPTY };

	// check every bus;device;function of PCI controller
	for( pci.bus = 0; pci.bus < 16; pci.bus++ )
		for( pci.device = 0; pci.device < 16; pci.device++ )
			for( pci.function = 0; pci.function < 16; pci.function++ ) {
				// retrieve class and subclass
				pci.result = driver_pci_read( DRIVER_PCI_REGISTER_class_and_subclass, pci ) >> 16;
