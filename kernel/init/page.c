...
void kernel_init_page( void ) {
	...
	for( uint16_t i = 0; i < limine_file_elf64_header -> h_entry_count; i++ ) {
		...
		// segment properties
		volatile uintptr_t segment_offset = MACRO_PAGE_ALIGN_DOWN( limine_file_elf64_header_entry[ i ].virtual_address ) - KERNEL_BASE_address;
		uint64_t segment_limit = (MACRO_PAGE_ALIGN_UP( limine_file_elf64_header_entry[ i ].virtual_address + limine_file_elf64_header_entry[ i ].memory_size ) - KERNEL_BASE_address) >> STD_SHIFT_PAGE;
		...
		kernel_page_map( kernel -> page_base_address, limine_kernel_address_request.response -> physical_base + segment_offset, KERNEL_BASE_address + segment_offset, segment_limit, segment_flags );
	}
