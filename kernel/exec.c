...
int64_t kernel_exec( uint8_t *name, uint64_t length, uint8_t stream_flow ) {
	...
	// exec.file.id_storage = kernel -> storage_old_root_id;
	kernel_storage_old_file( (struct STD_FILE_OLD_STRUCTURE *) &exec.file );
	...
	kernel_storage_old_read( (struct STD_FILE_OLD_STRUCTURE *) &exec.file, exec.workbench_address );
