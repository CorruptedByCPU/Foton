...
int64_t kernel_library_load( uint8_t *name, uint64_t length ) {
	...
	library.socket = (struct KERNEL_STRUCTURE_VFS *) kernel_vfs_file_open( (struct KERNEL_STRUCTURE_STORAGE *) &kernel -> storage_base_address[ kernel -> storage_root ], path, path_length, EMPTY );
