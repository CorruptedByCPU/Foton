...
void kernel_module_load( uint8_t *name, uint64_t length ) {
	...
	// set file path name
	uint8_t path_default[] = "/system/lib/modules/";
	...
	if( ! (workbench = kernel_memory_alloc( MACRO_PAGE_ALIGN_UP( file.length_byte ) >> STD_SHIFT_PAGE )) ) {
		// close file socket
		kernel_vfs_file_close( socket );

		// release path to file
		kernel_memory_release( path, MACRO_PAGE_ALIGN_UP( length + sizeof( path_default ) - 1 ) >> STD_SHIFT_PAGE );
