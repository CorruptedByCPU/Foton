...
void module_usb_hid_keyboard( uintptr_t kernel_ptr ) {
	...
	while( TRUE ) {
		...
		for( uint8_t i = 0; i < module_usb_controller_count; i++ ) {
			...
			uint8_t status = module_usb_descriptor_io( i, 0x08, (uintptr_t) cache & ~KERNEL_PAGE_mirror, MODULE_USB_UHCI_TD_PACKET_IDENTIFICATION_in );
...
void module_usb_hid_mouse( uintptr_t kernel_ptr ) {
	...
	while( TRUE ) {
		...
		for( uint8_t i = 0; i < module_usb_controller_count; i++ ) {
			...
			// create input Transfer Descriptor
			module_usb_descriptor_io( i, 0x04, (uintptr_t) cache & ~KERNEL_PAGE_mirror, MODULE_USB_UHCI_TD_PACKET_IDENTIFICATION_in );
...
uint8_t module_usb_detect_uhci( uint8_t i ) {
	...
	// reset controller
	for( uint8_t t = 0; t < 5; t++ ) { driver_port_out_word( module_usb_controller[ i ].base_address + offsetof( struct MODULE_USB_STRUCTURE_UHCI_REGISTER, command ), 0x0004 ); kernel -> time_sleep( 16 ); driver_port_out_word( module_usb_controller[ i ].base_address + offsetof( struct MODULE_USB_STRUCTURE_UHCI_REGISTER, command ), 0x0000 ); }

	// command register contains default value?
	if( driver_port_in_word( module_usb_controller[ i ].base_address + offsetof( struct MODULE_USB_STRUCTURE_UHCI_REGISTER, command ) ) ) return FALSE;	// no

	// status register contains default value?
	if( driver_port_in_word( module_usb_controller[ i ].base_address + offsetof( struct MODULE_USB_STRUCTURE_UHCI_REGISTER, status ) ) != 0x0020 ) return FALSE;	// no

	// clear status register
	driver_port_out_word( module_usb_controller[ i ].base_address + offsetof( struct MODULE_USB_STRUCTURE_UHCI_REGISTER, status ), 0x00FF );

	// modify register contains default value?
	if( driver_port_in_byte( module_usb_controller[ i ].base_address + offsetof( struct MODULE_USB_STRUCTURE_UHCI_REGISTER, start_of_frame_modify ) ) != 0x40 ) return FALSE;	// no

	// check command register function
	driver_port_out_word( module_usb_controller[ i ].base_address + offsetof( struct MODULE_USB_STRUCTURE_UHCI_REGISTER, command ), 0x0002 ); kernel -> time_sleep( 64 );

	// test passed?
	if( driver_port_in_word( module_usb_controller[ i ].base_address + offsetof( struct MODULE_USB_STRUCTURE_UHCI_REGISTER, command ) ) & 0x0002 ) return FALSE;	// no
...
void module_usb_uhci_queue( uint32_t *frame_list ) {
	...
	module_usb_queue_1u = (struct MODULE_USB_STRUCTURE_UHCI_QUEUE *) module_usb_queue_empty();

	// acquire 8/1024u query
	module_usb_queue_8u = (struct MODULE_USB_STRUCTURE_UHCI_QUEUE *) module_usb_queue_empty();

	// acquire 16/1024u query
	module_usb_queue_16u = (struct MODULE_USB_STRUCTURE_UHCI_QUEUE *) module_usb_queue_empty();
...
uint16_t module_usb_port_reset( uint8_t id ) {
	...
	// send command, reset
	driver_port_out_word( module_usb_controller[ module_usb_port[ id ].controller_id ].base_address + offsetof( struct MODULE_USB_STRUCTURE_UHCI_REGISTER, port[ module_usb_port[ id ].port_id ] ), driver_port_in_word( module_usb_controller[ module_usb_port[ id ].controller_id ].base_address + offsetof( struct MODULE_USB_STRUCTURE_UHCI_REGISTER, port[ module_usb_port[ id ].port_id ] ) ) & ~MODULE_USB_UHCI_PORT_STATUS_AND_CONTROL_port_reset ); kernel -> time_sleep( 64 );
	driver_port_out_word( module_usb_controller[ module_usb_port[ id ].controller_id ].base_address + offsetof( struct MODULE_USB_STRUCTURE_UHCI_REGISTER, port[ module_usb_port[ id ].port_id ] ), driver_port_in_word( module_usb_controller[ module_usb_port[ id ].controller_id ].base_address + offsetof( struct MODULE_USB_STRUCTURE_UHCI_REGISTER, port[ module_usb_port[ id ].port_id ] ) ) & ~MODULE_USB_UHCI_PORT_STATUS_AND_CONTROL_port_reset ); kernel -> time_sleep( 16 );

	// connection status
	for( uint8_t i = 0; i < 10; i++ ) {
		// retrieve port status
		status = driver_port_in_word( module_usb_controller[ module_usb_port[ id ].controller_id ].base_address + offsetof( struct MODULE_USB_STRUCTURE_UHCI_REGISTER, port[ module_usb_port[ id ].port_id ] ) );
		...
		// port status change?
		if( ((status & MODULE_USB_UHCI_PORT_STATUS_AND_CONTROL_port_enable_change) || (status & MODULE_USB_UHCI_PORT_STATUS_AND_CONTROL_connect_status_change)) ) {
			// clean it
			driver_port_out_word( module_usb_controller[ module_usb_port[ id ].controller_id ].base_address + offsetof( struct MODULE_USB_STRUCTURE_UHCI_REGISTER, port[ module_usb_port[ id ].port_id ] ), status & ~(MODULE_USB_UHCI_PORT_STATUS_AND_CONTROL_port_enable_change | MODULE_USB_UHCI_PORT_STATUS_AND_CONTROL_connect_status_change));
		...
		// try to enable port
		driver_port_out_word( module_usb_controller[ module_usb_port[ id ].controller_id ].base_address + offsetof( struct MODULE_USB_STRUCTURE_UHCI_REGISTER, port[ module_usb_port[ id ].port_id ] ), status | MODULE_USB_UHCI_PORT_STATUS_AND_CONTROL_port_enabled );
	}
	...
	for( uint8_t i = 0; i < module_usb_controller_count; i++ ) {
		...
		if( module_usb_controller[ i ].base_address ) {	// yes
			...
			if( module_usb_detect_uhci( i ) ) {
				...
				driver_port_out_word( module_usb_controller[ i ].base_address + offsetof( struct MODULE_USB_STRUCTURE_UHCI_REGISTER, interrupt_enable ), 0x000F );

				// reset frame number
				driver_port_out_word( module_usb_controller[ i ].base_address + offsetof( struct MODULE_USB_STRUCTURE_UHCI_REGISTER, frame_number ), EMPTY );

				// alloc area for frame list
				module_usb_controller[ i ].frame_base_address = kernel -> memory_alloc_low( TRUE ) & ~KERNEL_PAGE_mirror;
				driver_port_out_dword( module_usb_controller[ i ].base_address + offsetof( struct MODULE_USB_STRUCTURE_UHCI_REGISTER, frame_list_base_address ), module_usb_controller[ i ].frame_base_address );
				...
				driver_port_out_word( module_usb_controller[ i ].base_address + offsetof( struct MODULE_USB_STRUCTURE_UHCI_REGISTER, status ), 0xFFFF );

				// start UHCI controller
				driver_port_out_word( module_usb_controller[ i ].base_address + offsetof( struct MODULE_USB_STRUCTURE_UHCI_REGISTER, command ), TRUE | 1 << 6 );
				...
				for( uint16_t j = 0; j < module_usb_controller[ i ].size_byte >> 4; j++ ) {	// thats a proper way of calculating amount of available ports inside controller, but for UHCI only
					...
					module_usb_descriptor( module_usb_port_count, 0x08, descriptor_default & ~KERNEL_PAGE_mirror, MODULE_USB_UHCI_TD_PACKET_IDENTIFICATION_in, (uintptr_t) packet );
					...
					module_usb_descriptor( module_usb_port_count, module_usb_port[ module_usb_port_count ].default_descriptor_length, descriptor_default & ~KERNEL_PAGE_mirror, MODULE_USB_UHCI_TD_PACKET_IDENTIFICATION_in, (uintptr_t) packet );
					...
					module_usb_descriptor( module_usb_port_count, sizeof( struct MODULE_USB_STRUCTURE_DESCRIPTOR_CONFIGURATION ), descriptor_default & ~KERNEL_PAGE_mirror, MODULE_USB_UHCI_TD_PACKET_IDENTIFICATION_in, (uintptr_t) packet );
					...
					module_usb_descriptor( module_usb_port_count, descriptor_configuration -> total_length, descriptor_default & ~KERNEL_PAGE_mirror, MODULE_USB_UHCI_TD_PACKET_IDENTIFICATION_in, (uintptr_t) packet );
