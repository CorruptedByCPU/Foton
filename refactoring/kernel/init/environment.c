...
void kernel_init_environment( void ) {
	...
	for( uint64_t i = 0; i < limine_memmap_request.response -> entry_count; i++ ) {
		...
		if( sdt_length > limine_memmap_request.response -> entries[ i ] -> length ) continue;	// no

		// remember size for later use
		sdt_length = limine_memmap_request.response -> entries[ i ] -> length;
