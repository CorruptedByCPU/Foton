...
void kernel_module_load( uint8_t *name, uint64_t length ) {
	...
	struct KERNEL_STRUCTURE_IDT_RETURN *context = (struct KERNEL_STRUCTURE_IDT_RETURN *) (kernel_page_address( (uint64_t *) module -> cr3, KERNEL_STACK_pointer - STD_PAGE_byte ) + KERNEL_MEMORY_mirror + (STD_PAGE_byte - sizeof( struct KERNEL_STRUCTURE_IDT_RETURN )));
	...
	// kernel_page_map( (uint64_t *) module -> cr3, module_content & ~KERNEL_MEMORY_mirror, module_memory, module_page, KERNEL_PAGE_FLAG_present | KERNEL_PAGE_FLAG_write | (module -> page_type << KERNEL_PAGE_TYPE_offset) );

	// map module space to kernel space
	// kernel_page_map( (uint64_t *) kernel -> page_base_address, module_content & ~KERNEL_MEMORY_mirror, module_memory, module_page, KERNEL_PAGE_FLAG_present | KERNEL_PAGE_FLAG_write | (module -> page_type << KERNEL_PAGE_TYPE_offset) );
...
int64_t kernel_module_thread( uintptr_t function, uint8_t *name, uint64_t length ) {
	...
	// prepare Paging table for new process
	thread -> cr3 = kernel_memory_alloc_page() | KERNEL_MEMORY_mirror;
