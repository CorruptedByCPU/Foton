...
uintptr_t kernel_page_address( uint64_t *pml4, uintptr_t address ) {
	...
	uint64_t *pml3 = (uint64_t *) (MACRO_PAGE_ALIGN_DOWN( pml4[ (address >> KERNEL_PAGE_PML4_shift) & (KERNEL_PAGE_PMLx_entry - 1) ] ) | KERNEL_MEMORY_mirror);
	uint64_t *pml2 = (uint64_t *) (MACRO_PAGE_ALIGN_DOWN( pml3[ (address >> KERNEL_PAGE_PML3_shift) & (KERNEL_PAGE_PMLx_entry - 1) ] ) | KERNEL_MEMORY_mirror);
	uint64_t *pml1 = (uint64_t *) (MACRO_PAGE_ALIGN_DOWN( pml2[ (address >> KERNEL_PAGE_PML2_shift) & (KERNEL_PAGE_PMLx_entry - 1) ] ) | KERNEL_MEMORY_mirror);

	// return address of physical page connected to area
	return MACRO_PAGE_ALIGN_DOWN( pml1[ (address >> KERNEL_PAGE_PML1_shift) & (KERNEL_PAGE_PMLx_entry - 1) ] );
}

uint8_t kernel_page_alloc( uint64_t *pml4, uintptr_t address, uint64_t N, uint16_t flags ) {
	// start with following array[ entries ]
	uint16_t p4 = (address >> KERNEL_PAGE_PML4_shift) & (KERNEL_PAGE_PMLx_entry - 1); 
	uint16_t p3 = (address >> KERNEL_PAGE_PML3_shift) & (KERNEL_PAGE_PMLx_entry - 1);
	uint16_t p2 = (address >> KERNEL_PAGE_PML2_shift) & (KERNEL_PAGE_PMLx_entry - 1);
	uint16_t p1 = (address >> KERNEL_PAGE_PML1_shift) & (KERNEL_PAGE_PMLx_entry - 1);
		...
		for( ; p3 < KERNEL_PAGE_PMLx_entry; p3++ ) {
			...
			for( ; p2 < KERNEL_PAGE_PMLx_entry; p2++ ) {
				...
				for( ; p1 < KERNEL_PAGE_PMLx_entry; p1++ ) {
					...
					// allocate page to a logical address
...
uint8_t kernel_page_clang( uint64_t *pml4, uintptr_t source_address, uintptr_t target_address, uint64_t N, uint16_t flags ) {
	...
	// start with following array[ entries ]
	uint16_t p4 = (target_address >> KERNEL_PAGE_PML4_shift) & (KERNEL_PAGE_PMLx_entry - 1); 
	uint16_t p3 = (target_address >> KERNEL_PAGE_PML3_shift) & (KERNEL_PAGE_PMLx_entry - 1);
	uint16_t p2 = (target_address >> KERNEL_PAGE_PML2_shift) & (KERNEL_PAGE_PMLx_entry - 1);
	uint16_t p1 = (target_address >> KERNEL_PAGE_PML1_shift) & (KERNEL_PAGE_PMLx_entry - 1);
	...
	for( ; p4 < KERNEL_PAGE_PMLx_entry; p4++ ) {
		...
		if( ! pml4[ p4 ] ) {
			...
			pml3 = (uint64_t *) (pml4[ p4 ] | KERNEL_MEMORY_mirror);
		...
		for( ; p3 < KERNEL_PAGE_PMLx_entry; p3++ ) {
			...
			if( ! pml3[ p3 ] ) {
				...
				pml2 = (uint64_t *) (pml3[ p3 ] | KERNEL_MEMORY_mirror);
			...
			for( ; p2 < KERNEL_PAGE_PMLx_entry; p2++ ) {
				...
				if( ! pml2[ p2 ] ) {
					...
					pml1 = (uint64_t *) (pml2[ p2 ] | KERNEL_MEMORY_mirror);
...
uintptr_t kernel_page_remove( uint64_t *pml4, uintptr_t address ) {
	...
	uintptr_t removed = pml1[ (address >> KERNEL_PAGE_PML1_shift) & (KERNEL_PAGE_PMLx_entry - 1) ];
	...
	pml1[ (address >> KERNEL_PAGE_PML1_shift) & (KERNEL_PAGE_PMLx_entry - 1) ] = EMPTY;
