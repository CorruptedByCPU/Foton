...
void _entry( uintptr_t kernel_ptr ) {
	...
	// local semaphores, just to know if we need to execute thread for specific device
	// uint64_t semaphore_virtio_network	= FALSE;
	uint64_t semaphore_virtio_block		= FALSE;
	...
	for( uint16_t b = 0; b < 256; b++ )
		for( uint8_t d = 0; d < 32; d++ )
			for( uint8_t f = 0; f < 8; f++ ) {
				...
				// request device driver thread
				switch( module_virtio[ module_virtio_limit ].type ) {
					// case MODULE_VIRTIO_TYPE_network: { semaphore_virtio_network = TRUE; break; }
					case MODULE_VIRTIO_TYPE_block: { semaphore_virtio_block = TRUE; break; }
					default: { break; }
				}

				// do we have driver for that device?
				if( ! module_virtio[ module_virtio_limit ].type ) continue;	// no

				// debug
				kernel -> log( (uint8_t *) "[Virt I/O] " );
				switch( module_virtio[ module_virtio_limit ].type ) {
					case MODULE_VIRTIO_TYPE_block: { kernel -> log( (uint8_t *) "Block " ); break; }
					default: { kernel -> log( (uint8_t *) "Unsupported " ); break; }
				}
				kernel -> log( (uint8_t *) "Controller found at %2X:%2X:%u (device 0x%X), ", module_virtio[ module_virtio_limit ].pci.bus, module_virtio[ module_virtio_limit ].pci.device, module_virtio[ module_virtio_limit ].pci.function, device_vendor >> STD_MOVE_WORD );
				kernel -> log( (uint8_t *) "base address 0x%X, ", module_virtio[ module_virtio_limit ].base_address );
				kernel -> log( (uint8_t *) "IRQ %u, ", module_virtio[ module_virtio_limit ].irq );
				if( module_virtio[ module_virtio_limit ].semaphore_legacy ) kernel -> log( (uint8_t *) "Legacy\n" );
				else if( module_virtio[ module_virtio_limit ].semaphore_modern ) kernel -> log( (uint8_t *) "Modern\n" );
				else if( module_virtio[ module_virtio_limit ].semaphore_transitional ) kernel -> log( (uint8_t *) "Transitional\n" );
	...
	// any Virtio device?
	if( module_virtio_limit ) {
		// connect default interrupt handler
		kernel -> idt_attach( KERNEL_IDT_IRQ_offset + module_virtio[ EMPTY ].irq, KERNEL_IDT_TYPE_irq, (uintptr_t) module_virtio_irq_entry );

		// connect interrupt vector from IDT table to IOAPIC controller
		kernel -> io_apic_attach( KERNEL_IDT_IRQ_offset + module_virtio[ EMPTY ].irq, KERNEL_IO_APIC_iowin + (module_virtio[ EMPTY ].irq * 0x02) );

		// // network device found?
		// if( semaphore_virtio_network ) {
