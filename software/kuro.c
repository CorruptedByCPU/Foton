	...
	uint64_t e = EMPTY; while( vfs[ ++e ].name_limit ) {
		...
		switch( vfs[ e ].type ) {
			...
			case STD_FILE_TYPE_directory: {
				...
				if( ! kuro_icon[ KURO_MIMETYPE_directory ] ) kuro_icon_register( KURO_MIMETYPE_directory, (uint8_t *) "/var/gfx/icons/folder-green.tga" );
			...
			default: {
				...
				if( ! kuro_icon[ KURO_MIMETYPE_unknown ] ) kuro_icon_register( KURO_MIMETYPE_unknown, (uint8_t *) "/var/gfx/icons/unknown.tga" );
				...
				if( vfs[ e ].limit ) {	// no
					...
					if( lib_elf_identify( (uintptr_t) elf ) ) {	// yes
						...
						if( elf -> type == LIB_ELF_TYPE_shared_object ) {
							...
							if( ! kuro_icon[ KURO_MIMETYPE_library ] ) kuro_icon_register( KURO_MIMETYPE_library, (uint8_t *) "/var/gfx/icons/application-x-sharedlib.tga" );
						...
						// module?
							...
							if( ! kuro_icon[ KURO_MIMETYPE_module ] ) kuro_icon_register( KURO_MIMETYPE_module, (uint8_t *) "/var/gfx/icons/application-octet-stream.tga" );
						...
						// executable
							...
							// load module icon, if not present
							if( ! kuro_icon[ KURO_MIMETYPE_executable ] ) kuro_icon_register( KURO_MIMETYPE_executable, (uint8_t *) "/var/gfx/icons/application-x-executable.tga" );
				...
				if( entry[ local_list_entry_count ].name_length > 4 && lib_string_compare( (uint8_t *) &entry[ local_list_entry_count ].name[ entry[ local_list_entry_count ].name_length - 4 ], (uint8_t *) ".tga", 4 ) ) {
					...
					if( ! kuro_icon[ KURO_MIMETYPE_image ] ) kuro_icon_register( KURO_MIMETYPE_image, (uint8_t *) "/var/gfx/icons/image.tga" );
				...
				if(
					...
					if( ! kuro_icon[ KURO_MIMETYPE_plain_text ] ) kuro_icon_register( KURO_MIMETYPE_plain_text, (uint8_t *) "/var/gfx/icons/text-plain.tga" );
				...
				if(
					...
					// load text icon, if not present
					if( ! kuro_icon[ KURO_MIMETYPE_3d_object ] ) kuro_icon_register( KURO_MIMETYPE_3d_object, (uint8_t *) "/var/gfx/icons/3d.tga" );
	...
	while( storage -> type ) {
		...
		switch( entry[ local_list_entry_count ].type ) {
			case STD_STORAGE_TYPE_memory: {
				...
				if( ! kuro_icon[ KURO_MIMETYPE_memory ] ) kuro_icon_register( KURO_MIMETYPE_memory, (uint8_t *) "/var/gfx/icons/media-memory.tga" );
			...
			case STD_STORAGE_TYPE_disk: {
				...
				if( ! kuro_icon[ KURO_MIMETYPE_disk ] ) kuro_icon_register( KURO_MIMETYPE_disk, (uint8_t *) "/var/gfx/icons/drive-harddisk.tga" );
...
int64_t _main( uint64_t argc, uint8_t *argv[] ) {
	...
	kuro_icon_register( KURO_MIMETYPE_up, (uint8_t *) "/var/gfx/icons/go-up.tga" );
