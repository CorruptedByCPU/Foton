...
void wm_cursor( void ) {
	...
	// properties of areas
	uint32_t *source = (uint32_t *) ((uintptr_t) wm -> cursor -> descriptor + sizeof( struct LIB_WINDOW_DESCRIPTOR ));
	uint32_t *target = (uint32_t *) ((uintptr_t) wm -> canvas.descriptor + sizeof( struct LIB_WINDOW_DESCRIPTOR ));
