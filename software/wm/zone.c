...
void wm_zone_insert( struct WM_STRUCTURE_ZONE *current, uint8_t object ) {
	...
	if( current -> x > wm_object_cache.width - 1 ) return;
	if( current -> y > wm_object_cache.height - 1 ) return;
	...
	struct WM_STRUCTURE_ZONE *zone = &wm_zone_base_address[ wm_zone_limit ];
	...
	if( current -> x < 0 ) {
		...
		zone -> x = 0;
	} else if( current -> x + current -> width > wm_object_cache.width ) {
		// right side
		zone -> x = current -> x;
		zone -> width = current -> width - ((current -> x + current -> width) - (int16_t) wm_object_cache.width);
	} else {
		// whole zone
		zone -> x = current -> x;
	...
	if( current -> y < 0 ) {
		...
		zone -> y = 0;
	} else if( current -> y + current -> height > wm_object_cache.height ) {
		// bottom side
		zone -> y = current -> y;
		zone -> height = current -> height - ((current -> y + current -> height) - (int16_t) wm_object_cache.height);
	} else {
		// whole zone
		zone -> y = current -> y;
...
void wm_zone( void ) {
	...
	struct WM_STRUCTURE_ZONE *zone = wm_zone_base_address;
