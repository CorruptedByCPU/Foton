...
void wm_init( void ) {
	// obtain information about framebuffer
	...
	// as local object
	wm_object_cache = (struct WM_STRUCTURE_OBJECT *) std_memory_alloc( MACRO_PAGE_ALIGN_UP( sizeof( struct WM_STRUCTURE_OBJECT ) ) >> STD_SHIFT_PAGE );
	...
	// for( uint64_t y = INIT; y < wm_object_cache -> height; y++)
	// 	for( uint64_t x = INIT; x < wm_object_cache -> width; x++ )
	// 		cache_pixel[ (y * wm_object_cache -> width) + x ] = 0x00FF0000;
	...
	wm_object_workbench = wm_object_create( INIT, INIT, wm_object_cache -> width, wm_object_cache -> height );
