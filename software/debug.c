...
int64_t _main( uint64_t argc, uint8_t *argv[] ) {
	...
	struct STD_STRUCTURE_IPC_WINDOW_DESCRIPTOR *answer = EMPTY;
	...
	answer = (struct STD_STRUCTURE_IPC_WINDOW_DESCRIPTOR *) &wm_data;
	if( ! answer -> descriptor ) { print( "Window Manager denied request." ); exit(); }
	struct STD_STRUCTURE_WINDOW_DESCRIPTOR *window = (struct STD_STRUCTURE_WINDOW_DESCRIPTOR *) answer -> descriptor;

	struct STD_STRUCTURE_NETWORK_INTERFACE eth0;
	std_network_interface( (struct STD_STRUCTURE_NETWORK_INTERFACE *) &eth0 );

	uint32_t *pixel = (uint32_t *) ((uintptr_t) window + sizeof( struct STD_STRUCTURE_WINDOW_DESCRIPTOR ));
	...
	while( TRUE ) {
		...
		struct STD_STRUCTURE_NETWORK_INTERFACE tmp;
		std_network_interface( (struct STD_STRUCTURE_NETWORK_INTERFACE *) &tmp );
