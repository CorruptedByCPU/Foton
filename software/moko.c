...
int64_t _main( uint64_t argc, uint8_t *argv[] ) {
	...
	if( file.length ) std_file( (struct STD_FILE_OLD_STRUCTURE *) &file );
	...
	if( file.id ) {
		...
		std_file_read( (struct STD_FILE_OLD_STRUCTURE *) &file, (uintptr_t) document_area );
	} else {
		...
		if( key_ctrl_semaphore ) {
			switch( key ) {
				...
				case 'o': {
					...
					// retrieve file name
					struct STD_FILE_OLD_STRUCTURE save_as = { EMPTY };
					...
					while( ! saved ) {
						...
						if( save_as.length ) std_file( (struct STD_FILE_OLD_STRUCTURE *) &save_as );
						...
						// if( saved ) std_file_write( (struct STD_FILE_OLD_STRUCTURE *) &save_as, (uintptr_t) document_area, document_size );
