...
void init( void ) {
	...
	rgl = lib_rgl( d3_interface.descriptor -> width - (LIB_INTERFACE_BORDER_pixel << STD_SHIFT_2), d3_interface.descriptor -> height - (LIB_INTERFACE_HEADER_HEIGHT_pixel + LIB_INTERFACE_BORDER_pixel), d3_interface.descriptor -> width, (uint32_t *) ((uintptr_t) d3_interface.descriptor + sizeof( struct STD_WINDOW_STRUCTURE_DESCRIPTOR ) + (((LIB_INTERFACE_HEADER_HEIGHT_pixel * d3_interface.width) + LIB_INTERFACE_BORDER_pixel) << STD_VIDEO_DEPTH_shift)) );
