...
void init( void ) {
	...
	rgl = lib_rgl( d3_interface.width - ((LIB_INTERFACE_BORDER_pixel + d3_interface.descriptor -> offset) << STD_SHIFT_2),
	d3_interface.height - (LIB_INTERFACE_HEADER_HEIGHT_pixel + LIB_INTERFACE_BORDER_pixel + (d3_interface.descriptor -> offset << STD_SHIFT_2)), d3_interface.width, (uint32_t *) ((uintptr_t) d3_interface.descriptor + sizeof( struct STD_WINDOW_STRUCTURE_DESCRIPTOR ) + ((((LIB_INTERFACE_HEADER_HEIGHT_pixel + d3_interface.descriptor -> offset) * d3_interface.width) + LIB_INTERFACE_BORDER_pixel + d3_interface.descriptor -> offset) << STD_VIDEO_DEPTH_shift)) );
