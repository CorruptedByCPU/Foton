...
uint64_t _main( uint64_t argc, uint8_t *argv[] ) {
	...
	de_window_taskbar = lib_window( 0, framebuffer.height_pixel - DE_TASKBAR_HEIGHT_pixel, framebuffer.width_pixel, DE_TASKBAR_HEIGHT_pixel );
	...
	if( (cursor_file = fopen( (uint8_t *) "/usr/share/media/cursor/default.tga", EMPTY )) ) {
		...
		de_window_cursor = lib_window( framebuffer.width_pixel >> STD_SHIFT_2, framebuffer.height_pixel >> STD_SHIFT_2, cursor_image -> width, cursor_image -> height );
	} else
